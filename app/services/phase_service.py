...
def enforce_phase_order(project_id: str, phase_name: str):
    if phase_name == "morphological_chart":
        ...
        if not prev.data or not prev.data[0].get("human_approved_data"):
            raise ValueError("Phase 1 must be human approved before proceeding to Phase 2.")
        ...
        if not prev.data or not prev.data[0].get("human_approved_data"):
            raise ValueError("Phase 2 must be human approved before proceeding to Phase 3.")
